...
The autocrop mechanism is also utilized to be as close to a practical application as possible.
Furthermore, inference starts over 500 frames before the annotated sequence, giving crop coordinates time to converge to their optimal positions.
...
The evaluation includes all models using the original and the adapted autocrop to ensure thoroughness.

Since sequences also include frames with \ac{GT} in which the train is located before the switch, these frames are excluded.
...
The original model with the original autocrop mechanism of \cite{tepNet2024} is in blue.
Then, the adapted single-frame-based model with the adapted and the original autocrop (ogA) is shown in yellow and green.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcccc}
        ...
        \hline
        \rowcolor[gray]{0.9} TEP Net \cite{tepNet2024} IoU & \textbf{94.76 \%}            & 18.47 \%          & 63.96 \%              & 22.10 \%        \\
        ...
        \rowcolor[gray]{0.9} best temporal model IoU       & 88.23 \%            & \textbf{23.43 \%}          & \textbf{83.77 \%}              & \textbf{40.59 \%}        \\
        \hline
    \end{tabular}
    }
    \caption{Temporal model results on test switches}
...
The \textbf{Switch 1} sequence is such a scenario.
Therefore, it is similar to a rail without any switches.
The reason the original model performed with such high accuracy lies mainly in the original autocrop mechanism.
...
Therefore, especially in those two scenes, using the original autocrop is advantageous.
...
Therefore, the additional average of only these two scenes is included in \autoref{tab:temporalModelsResults}.
...
When excluding those two scenes as expected, a temporal model with the adapted autocrop mechanism (CNN\_FC\_FCOUT\_V1) outperforms other techniques.
...
\begin{table}[H]
    ...
    \caption{Temporal models results with \ac{IoU}s for the four switch sequences that are in the test set.}
...
\begin{table}[H]
    ...
    \caption{Results of temporal models including parameters, flops, MACs and latencies.}
...
In this study, no temporal models utilizing a MobileNet backbone are examined regarding their accuracy.

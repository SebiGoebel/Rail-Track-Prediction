...
Datasets are essential for developing autonomous driving systems, particularly for training and testing algorithms or neural networks.
...
A common approach to solving problems in autonomous driving systems is using vision-based machine learning algorithms~\cite{railsem19dataset}.
These applications typically rely on camera-based data to address various challenges~\cite{railsem19dataset}.

Since autonomous vehicles are increasingly considered a groundbreaking technology of the future \cite{FraunhoferInstituteforCognitiveSystemsIKS}, a lot of work is put into developing such systems.
However, this quickly evolving field often focuses on road vehicles like cars or trucks.
Therefore, most publicly available datasets focus on this application and primarily reflect scenarios in road traffic \cite{railsem19dataset}.

\subsubsection{Classification Datasets}

There are a couple of public datasets for different \ac{CV} applications. The data of the following datasets are gathered for classification tasks.

% ---------------------------------------- Table: Datasets for Classification ----------------------------------------
\begin{table}[H]
\centering
\caption{Datasets for Classification}\label{tab_datsets_classification}
\begin{tabular}{| p{0.3\linewidth} | p{0.25\linewidth} | p{0.35\linewidth} |}\hline
\textbf{Dataset} & \textbf{Labels} & \textbf{Number of relevant images}\\\hline
...
\noindent\textit{CIFAR 100} dataset \cite{cifar100} consists of small images with pixel size 32x32. This dataset includes 600 images with the label \textit{trains}.
...
Semantic segmentation labels are often referred to as dense or pixel-wise annotated data.
These datasets are characterized by each pixel in their images being assigned to a class.
...
\begin{tabular}{| p{0.3\linewidth} | p{0.3\linewidth} | p{0.35\linewidth} |}\hline
\textbf{Dataset} & \textbf{Labels} & \textbf{Number of relevant images}\\\hline
...
% Problems with general datasets
...
% Birds eye views and sign datasets

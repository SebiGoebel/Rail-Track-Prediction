...
An initial approach tries to combine object detection with semantic segmentation. In the first experiments, object detectors determine switch states. Due to unsatisfactory results, a fundamentally different solution is found. A lightweight model serves as the new baseline that regresses the two rails of the train's path. Enhanced robustness is achieved with an improved cropping mechanism responsible for input images and an adapted model architecture.
